    ...
    private fun initializePersistableDataStream(stream: Flux<out ElectricReading>) =
        stream
            ...
            .flatMap {
                ...
                eventPublisher.query(args, it) { ctx, reading -> PersistenceContext(ctx.key().first, ctx.key().second, reading, ctx) }
            }
            .flatMap { ctx ->
                logger.info { "initial reading for ${ctx.alias}.${ctx.deviceType} = ${ctx.reading}, setting up counter..." }
                eventPublisher.command(ReportingService.commandRequestCounter(ctx.alias, ctx.reading.toDouble()), ctx ) { c, id -> c.updateMeterId(id) }
            }
            .flatMap { ctx ->
    ...
    private fun initializeInstantDataStream(stream: Flux<out ElectricReading>) =
        stream
            ...
            .flatMap {
                ...
                eventPublisher.command(ReportingService.commandRequestGauge(it.key(), storage), it) { ctx, _ -> InstantContext(ctx.key(), storage, ctx) }
            }
            .flatMap { ctx ->
